...
> = {
  ...
  "zai-paas-search": (apiKey, providerSpecificData = {}) => {
    ...
    // open.bigmodel.cn, anything else → api.z.ai (international). Region is read
    // from providerSpecificData.apiRegion — same field name GLM uses. NOTE: the
    // handler additionally honors a per-request provider_options.apiRegion/baseUrl
